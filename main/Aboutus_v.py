...
#分类列表
def aboutus_lists(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":[]}
        msg['data'],_,_,_,_  = aboutus.page(aboutus, aboutus, {})
        return JsonResponse(msg)
...
# （按值统计）时间统计类型(多)
def aboutus_valueMul(request, xColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
...
# （按值统计(多)）
def aboutus_o_valueMul(request, xColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
...
def aboutus_security(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT * FROM aboutus where ='{0}'".format(req_dict['username'])
        record = {}
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
